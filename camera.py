...
def photobooth(chosen_prop, prop_type, camera, debug):
    ...
    def mouse_callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            if (button_position[0] <= x <= button_position[0] + button_size[0] and button_position[1] <= y <= button_position[1] + button_size[1]):
                ...
                cv2.imwrite("captured_image.png", frame_no_buttons)
    ...
    with mp_face_mesh.FaceMesh(min_detection_confidence=0.5, min_tracking_confidence=0.5) as face_mesh:
        while cap.isOpened():
            ...
            # resize the frame for faster processing time
            frame = cv2.resize(frame, (854, 480))
    
            frame.flags.writeable = False
    
            # process with media pipe
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    ...
                    # Convert face_landmarks to NumPy array for easier indexes
                    ...
                    # drar circles on specifc landmarks
                    if debug == 1:
            ...
            # Create a copy of the frame before drawing the buttons
            frame_no_buttons = frame.copy()
